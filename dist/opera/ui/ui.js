...
(function () {
  ...
  document.getElementById('btn-planner').onclick = function () {
    var plannerUrl = chrome.runtime.getURL('ui/planner/planner.html');
  ...
  document.getElementById('btn-collapse-all').onclick = uiToggleCollapseAll;
  ...
  document.getElementById('btn-import').onclick = function () {
    var hasData = Object.keys(UIState.currentData).filter(function (k) {
    ...
    reader.onload = function (ev) {
      try {
        ...
        pbSaveData(imported, function () {
          ...
          uiRender(imported);
          uiSetStatus('✅ ' + matCount + ' materia(s) importadas', 'success');
        });
      } catch (err) {
        uiSetStatus('⚠️ Archivo .poli inválido o vacío', 'warning');
      }
    };
  ...
  document.getElementById('btn-export').onclick = function () {
    var exported = pbExportPoliFile(UIState.currentData, null);
    if (!exported) {
      uiSetStatus('⚠️ No hay datos para exportar', 'warning');
    } else {
      uiSetStatus('✅ Exportado', 'success');
    }
  };
  ...
  chrome.storage.onChanged.addListener(function (changes, area) {
    if (area === 'local' && changes[PB_STORAGE_KEY] && !UIState.isInternalChange) {
      uiRender(changes[PB_STORAGE_KEY].newValue);
    }
  });
