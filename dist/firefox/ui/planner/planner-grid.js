/* ============================================================
   Polibaldeo – ui/ui/planner/planner-grid.js
  ...
  if (PlannerState.viewMode === 'clases') {
    pbMatKeys(PlannerState.data).forEach(function(m) {
      ...
      pbParseH(pd.horariosTeoricos || pd.horarios || []).forEach(function(sl) {
        ...
        // safe: materia name and pId displayed in extension's own sandboxed page (chrome-extension://)
        blk.innerHTML  = '<div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">' + m + '</div><div style="opacity:.55;font-size:8.5px">P' + pId + '</div>'; // safe: sandboxed extension page
      ...
      if (pd.horariosPracticos && pd.horariosPracticos.length) {
        pbParseH(pd.horariosPracticos).forEach(function(sl) {
          ...
          blk.style.cssText =
            ...
            'border-left:3px solid ' + c.ac + ';';
          // safe: materia name and pId displayed in extension's own sandboxed page (chrome-extension://)
          blk.innerHTML = '<div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">' + m + ' (Prác.)</div><div style="opacity:.55;font-size:8.5px">P' + pId + '</div>'; // safe: sandboxed extension page
...
function plannerDrawExamBlocks() {
  ...
  pbMatKeys(PlannerState.data).forEach(function(m) {
    ...
    blk.style.cssText =
      ...
      'border-left:3px solid ' + c.ac + ';';
    // safe: materia name and pId displayed in extension's own sandboxed page (chrome-extension://)
    blk.innerHTML  = '<div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">' + m + ' (Examen)</div><div style="opacity:.55;font-size:8.5px">P' + pId + '</div>'; // safe: sandboxed extension page
