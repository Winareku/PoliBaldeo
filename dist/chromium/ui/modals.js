/* ============================================================
   Polibaldeo – ui/modals.js
   Modales para edición de paralelos, materias y confirmación.
   Depende de: state.js, toasts.js, modal.js
   ============================================================ */
...
function pbSectionHeader(title, sectionId, defaultOpen, extraHTML, onOpen) {
  ...
  // safe: static HTML template — icon names and section title are hardcoded strings in our callers
  hdr.innerHTML = `
...
function uiStartNameEdit(nameEl, card) {
  ...
  function finish(save) {
    ...
    restored.onclick   = function() { uiStartNameEdit(restored, card); };

    if (save && newName && newName !== oldName) {
      if (UIState.currentData[newName]) {
        uiSetStatus('⚠️ Ya existe una materia con ese nombre', 'warning');
        restored.textContent = oldName;
        restored.title       = oldName;
      } else {
        UIState.currentData[newName] = UIState.currentData[oldName];
        delete UIState.currentData[oldName];
        var ord = UIState.currentData._order;
        if (ord) {
        ...
        uiSaveVisualState();
        uiSetStatus('✅ Nombre actualizado', 'success');
        // Transferir color en _colorMap
        if (UIState.currentData._colorMap) {
          UIState.currentData._colorMap[newName] = UIState.currentData._colorMap[oldName] || uiColorMap[oldName];
          delete UIState.currentData._colorMap[oldName];
        }
        // Transferir color local
        uiColorMap[newName] = uiColorMap[oldName];
        delete uiColorMap[oldName];
...
function _pbNextParId(materia) {
  var mat  = UIState.currentData[materia];
...
function uiOpenEditModal(materia, pId, isNew) {
  ...
  if (!isNew && materia && pId) {
    parData = (UIState.currentData[materia] &&
               UIState.currentData[materia].paralelos &&
               UIState.currentData[materia].paralelos[pId]) || {};
  }
  ...
  if (addHorarioTeoBtn) {
    addHorarioTeoBtn.onclick = function() {
      ...
      // safe: static schedule row template — no user data, only fixed HTML structure with day/time inputs
      row.innerHTML = `
        <select class="pb-day-sel">
  ...
  modal.getElement('#modal-save').onclick = function() {
    ...
    if (!newPId) {
      uiSetStatus('⚠️ El paralelo no puede estar vacío', 'warning');
      return;
    }
    ...
    for (var i = 0; i < rowsTeo.length; i++) {
      ...
      if (!/^\d{1,2}:\d{2}$/.test(start) || !/^\d{1,2}:\d{2}$/.test(end)) {
        uiSetStatus('⚠️ Formato de hora inválido en ' + day + ' (usa HH:MM)', 'warning');
        return;
      }
      ...
      if (eMins <= sMins) {
        uiSetStatus('⚠️ En el ' + day + ', la hora de fin debe ser mayor a la de inicio', 'warning');
        return;
      }
      ...
      for (var j = 0; j < daySchedules[day].length; j++) {
        ...
        if (sMins < block.e && eMins > block.s) {
          uiSetStatus('⚠️ Hay un cruce de horas el día ' + day, 'warning');
          return;
        }
      }
    ...
    if (containerPrac) {
      ...
      for (var k = 0; k < rowsPrac.length; k++) {
        ...
        if (eMinsP <= sMinsP) {
          uiSetStatus('⚠️ En el ' + day + ' (práctico), la hora de fin debe ser mayor a la de inicio', 'warning');
          return;
        }
    ...
    if (isNew) {
      if (mat.paralelos[newPId]) {
        uiSetStatus('⚠️ Ya existe ese número de paralelo', 'warning');
        return;
      }
    ...
    pbSaveData(UIState.currentData, function() {
      ...
      uiRender(UIState.currentData);
      uiSetStatus('✅ Paralelo guardado', 'success');
    });
  };
...
function uiOpenMateriaModal() {
  ...
  modal.getElement('#modal-save').onclick = function() {
    ...
    if (!name) {
      uiSetStatus('⚠️ El nombre no puede estar vacío', 'warning');
      return;
    }
    if (UIState.currentData[name]) {
      uiSetStatus('⚠️ Ya existe una materia con ese nombre', 'warning');
      return;
    }

    UIState.currentData[name] = {
    ...
    UIState.currentData._order.push(name);
    ...
    pbSaveData(UIState.currentData, function() {
      UIState.isInternalChange = false;
      uiRender(UIState.currentData);
      uiSetStatus('✅ Materia añadida', 'success');
    });
  };
