/* ============================================================
   Polibaldeo – ui/planner/planner-ui.js
...
function plannerRenderLeft() {
  ...
  mks.forEach(function(m) {
    ...
    credChip.className = 'chip chip-cred';
    credChip.textContent = (mat.creditos || 0) + '\u202fcr';

    var chevron = document.createElement('span');
    chevron.className = 'sym acc-chevron';
    chevron.textContent = 'expand_more';

    meta.appendChild(credChip);
    ...
    if (pKeys.length === 0) {
      ...
      cbEl.className = cbCls;
      var cbCheck = document.createElement('span');
      cbCheck.className = 'sym';
      cbCheck.textContent = 'check';
      cbEl.appendChild(cbCheck);

      // Content
      var contentEl = document.createElement('div');
      contentEl.className = 'par-content';

      var badge = document.createElement('span');
      badge.className = 'par-badge';
      badge.style.cssText = 'background:' + c.ac + '1a; color:' + c.fg + '; border:1px solid ' + c.ac + '40';
      badge.textContent = 'Par. ' + pId;
      contentEl.appendChild(badge);

      if (prof) {
        var profEl = document.createElement('div');
        profEl.className = 'par-prof';
        profEl.textContent = prof;
        contentEl.appendChild(profEl);
      }
      if (ubicTeo) {
        var ubicEl = document.createElement('div');
        ubicEl.className = 'par-ubic';
        ubicEl.textContent = ubicTeo;
        contentEl.appendChild(ubicEl);
      }
      if (ubicPrac) {
        var ubicPracEl = document.createElement('div');
        ubicPracEl.className = 'par-ubic-prac';
        ubicPracEl.textContent = ubicPrac;
        contentEl.appendChild(ubicPracEl);
      }

      var slotsEl = document.createElement('div');
      slotsEl.className = 'par-slots';
      // slot-pill strings are controlled format strings (e.g. 'LUN 08:00-10:00'), not arbitrary HTML
      slotsEl.innerHTML = slotsHtml || '<span class="slot-pill">Sin horario</span>'; // safe: formatted schedule strings
      contentEl.appendChild(slotsEl);

      item.appendChild(cbEl);
      item.appendChild(contentEl);

      if (hasConflict) {
        var warnIcon = document.createElement('span');
        warnIcon.className = 'sym conflict-icon';
  ...
  getAccordions().forEach(function(acc) {
    ...
    getParaleloItems(acc).forEach(function(item) {
      ...
      if (slotsContainer) {
        ...
        // safe: horario strings are controlled format strings (e.g. 'LUN 08:00-10:00'), not arbitrary HTML
        slotsContainer.innerHTML = slotsHtml || '<span class="slot-pill">Sin horario</span>'; // safe: formatted schedule strings
      }
